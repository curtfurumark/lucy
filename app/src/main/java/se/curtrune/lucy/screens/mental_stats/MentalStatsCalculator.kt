...
class MentalStatsCalculator(val items: List<Item>) {
    ...
    val sumAnxiety = items.stream().mapToInt(Item::getAnxiety).sum()
    val sumMood = items.stream().mapToInt(Item::getMood).sum()
    val sumEnergy = items.stream().mapToInt(Item::getEnergy).sum()
   ...
   // val sumHappiness = items.stream().mapToInt(Item::getHappiness).sum()
    ...
    val sumStress = items.stream().mapToInt(Item::getStress).sum()
...
data class MentalDate(val date: LocalDate, val items: List<Item>){
    val energy = items.stream().mapToInt(Item::getEnergy).sum()
    val mood = items.stream().mapToInt( Item::getMood).sum()
    val anxiety = items.stream().mapToInt(Item::getAnxiety).sum()
    val stress = items.stream().mapToInt(Item::getStress).sum()
